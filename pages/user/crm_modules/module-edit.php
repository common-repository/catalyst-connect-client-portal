...
*/
    ...
    $exFldHstry = array();
    if(!isset($allLayout)){
        $exView = $wpdb->get_row("SELECT * FROM ccgclientportal_options WHERE option_name = 'crmModules' AND option_index LIKE 'lay_Edit_".$rl.$module."'");
                ...
                if(isset($_POST['csrf_token_nonce']) && wp_verify_nonce($_POST['csrf_token_nonce'],'updateModule')){
                    foreach ($layViewV['fieldName'] as $flfapinm) {
                        ...
                        if(isset($fieldList[$fldApiN]['data_type'])){
                            ...
                            elseif($fdtype == 'datetime')$_POST[$fldApiN] = date("c", strtotime($value));
                                ...
                                $fieldListSF = $CrmModules->getSubformFieldList($module.'__'.$fldApiN);
                    ...
                    $zohoArr = array($_POST);
                    $update = $CPNzoho->updateRecordsN($crmId, $zohoArr, $module, 'user', 'yes');
                    ...
                    // $error = true;
                    $errorm = "Somethings went wrong. Please contact your administrator.";
                }        
            }
...
<div id="portal-cotenier">
    <div class="col-md-12">
        <div class="<?php echo $module;?>-page page-body">
            ...
            <?php if($error){ ?>
                <div class="alert alert-dismissible alert-danger">
                  <button type="button" class="close" data-dismiss="alert">&times;</button>
                  <strong>Oh snap!</strong> Contact does not update. Please try again later !!
                </div>
            <?php } ?>
            ...
            <?php }else{ ?> 
                ...
                <?php if(isset($layViewV['rowAdded'], $layViewV['layoutFldColumn'])){ ?>
                    ...
                    <?php if($recordDtl){ ?>
                        ...
                        <h4 class="section-title"><?php echo ($layViewV['sectiontitle'][0]) ? esc_html($layViewV['sectiontitle'][0]) : "&nbsp;";?></h4>
                            ...
                            if($fldEdit == "yes" && isset($fieldList[$fieldApiName]["view_type"]["edit"]) && $fieldList[$fieldApiName]["view_type"]["edit"] == true){
                                $ttlcol = $ttlcol + (int)$col;

                                    if(array_key_exists($fieldApiName, $recordDtl)){
                                        ...
                                        $fValue = date("Y-m-d", strtotime($fValue));
                                        }elseif(($type == 'datetime') && (strtotime($fValue))){
                                        $fValue = date('Y-m-d\TH:i', strtotime(date("Y-m-d H:i:s", strtotime($fValue))));
                                                    ...
                                                    if($sfexf !=""){ ?>
                                                        ...
                                                        if(($type == 'multiselectpicklist') && is_array($fValue)){
                                                           ...
                                                           $fValue = date("Y-m-d", strtotime($fValue));
                                                            ...
                                                            if(is_array($fValue)) $fValue = "";
                                                            include 'defineSFInputTypeEdit.php';
                                                         ?>
                                                        </td>
                                                    <?php
                                                    } 
                        ...
                        <?php } ?>
                                                <?php }else{
                                                    ...
                                                    include 'defineInputTypeEdit.php';
                                                } ?>
                                            </div>
                                        </div>
                                    <?php } ?>
                                    ...
                                    <?php if($colN == $layViewV['columnAdded'][$rowN]) {
                                        ...
                                        <?php if(isset($layViewV['sectiontitle'][$rowN]) && $layViewV['sectiontitle'][$rowN]!=""){ ?>
                    ...
                    <?php } else { ?>
                        <p style="text-align: center;margin: 0px;">Record not found</p></td>
                    <?php } 
                } else { ?>
                    <p style="text-align: center;margin: 0px;">Customization for <?php echo esc_html(str_replace("_", " ", $module));?> is not complete. Please Contact your administrator. </p></td>                        
...
<style type="text/css">
    ...
    #showDialogLookup .modal-body .searchstr{width: calc(100% - 80px);float: left;}
